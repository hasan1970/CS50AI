...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for word1 in self.domains[x]:
            for word2 in self.domains[y]:
                if word1!=word2 and word1[x1]==word2[y1]:
                        break
            else:
                list1.append(word1)
    ...
    def ac3(self, arcs=None):
        ...
        for x,y in arcs:
            if self.revise(x,y) is True:
                for j in self.crossword.neighbors(x):
                    arcs.append((x,j))
    ...
    def order_domain_values(self, var, assignment):
        ...
        for nei in neighbor:
            ...
            for var_word in self.domains[var]:
                ...
                for nei_word in self.domains[nei]:
                    if var_word[overlap[0]]!=nei_word[overlap[1]]:
                        score+=1
                result[var_word]=score
        
    
            
        abc=sorted(result.items(), key=lambda x: x[1])
        list1=[]
        for elem in abc:
            list1.append(elem[0])
    ...
    def select_unassigned_variable(self, assignment):
        ...
        abc = sorted(dict2.items(), key=lambda x: x[1])
        list1=[]
        for elem in abc:
            list1.append(elem[0])
        if len(list1)==0:
